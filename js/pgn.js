...
function GetBestMovePy(best_move_py){
	bestmove = best_move_py;
	return bestmove;
}
...
function GetLichessSide() {
  return document.querySelector('.cg-wrap.orientation-white.manipulable') ? 1 : 0;
}
...
function SquareToPos(square, size) {
	...
	GetLichessSide() == 1 ? ranks.reverse() : files.reverse();
...
function DrawOnScreen() {
	...
	let boardElement = document.querySelector('.cg-wrap');
	...
	let turn = GetLichessSide();
	
		DrawArrow(svg, bestmove, turn, width);
...
function DrawArrow(svg, move, turn, size) {
	...
	marker.setAttribute('fill', colors[0]);
...
function SendMovesToServer() {
    ...
    fetch('http://localhost:5000/moves', requestOptions)
		...
		.then(best_move_py => {
			if (best_move_py != "null"){
				console.log(best_move_py);
				GetBestMovePy(best_move_py);
				DrawOnScreen();
			}
		})
        .catch(error => {
